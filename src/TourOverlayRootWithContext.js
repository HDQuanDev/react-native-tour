...
const TourOverlayContent = ({ step, targetRef, onStepPress, loopCount = 0, onDestroy, contextValue }) => {
...
class TourOverlayRootWithContext {
  constructor() {
    this.sibling = null;
  }

  show(step, targetRef, onStepPress, loopCount = 0, contextValue = {}) {
    this.destroy();
  ...
  update(step, targetRef, onStepPress, loopCount = 0, contextValue = {}) {
    if (step && targetRef) {
      if (this.sibling) {
        this.sibling.update(
          <TourOverlayContent
        ...
        this.show(step, targetRef, onStepPress, loopCount, contextValue);
      }
    } else {
      this.destroy();
    }
...
export default TourOverlayRootWithContext;
